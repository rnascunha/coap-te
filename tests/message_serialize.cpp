...
void serialize_parse_header_success_impl(
  ...
  auto size = msg::serialize_header(
                tp, co, mid, tk, coap_te::buffer(data), ec);
  EXPECT_FALSE(ec);
  ...
  if constexpr (Parse) {
    ...
    Message resp;
    auto size_p = msg::parse(coap_te::buffer(data, size), resp, ecp);
    EXPECT_FALSE(ecp);
...
TEST(CoAPMessage, SerializeParseHeaderSuccess) {
  ...
  }
  {
    ...
    serialize_parse_header_success<PARSEABLE_TYPES>(
                    ...
                    msg::code::put,
                    0x1234U,                         // message_id
                    coap_te::buffer(std::string_view{"teste"}),
                    4 + 5);
  }
  {
    ...
    serialize_parse_header_success<PARSEABLE_TYPES>(
                    ...
                    msg::code::put,
                    0x1234U,                         // message_id
                    coap_te::buffer(std::string_view{"teste12345678"}),
                    4 + 8);
  }
}
...
void serialize_parse_success_impl(
  ...
  size_s = serialize(req, mid, coap_te::buffer(data), ecs);
  EXPECT_FALSE(ecs);
  if (expected_size > 0) {
  ...
  coap_te::error_code ecp;
  MessageParse resp;
  auto size_p = msg::parse(coap_te::buffer(data, size_s), resp, ecp);
  EXPECT_FALSE(ecp);
...
TEST(CoAPMessage, SerializeParseMessage) {
  {
    ...
    serialize_parse_success<TYPES_COMBINE>(
                    ...
                    msg::code::put,
                    0x1234U,
                    coap_te::buffer(std::string_view{"teste"}),         // token
  ...
  }
  {
    ...
    serialize_parse_success<TYPES_COMBINE>(
                    ...
                    coap_te::buffer(std::string_view{"My very long long "
                                    "payload that I want"
