...
buffer_compare(one_buffer, one_buffer,
    ...
    Iterator2 buf2, Iterator2) noexcept {
...
buffer_compare(one_buffer, multiple_buffers,
  ...
  for (const_buffer buf1_buffer{buffer(*buf1_begin, max_size)};
       buf1_buffer.size() != 0 && buf2_iter != buf2_end;
       ++buf2_iter) {
    const_buffer buf2_buffer{*buf2_iter};
...
buffer_compare(multiple_buffers, multiple_buffers,
            Iterator1 buf1_begin, Iterator1 buf1_end,
            Iterator2 buf2_begin, Iterator2 buf2_end) noexcept {
  Iterator1 buf1_iter = buf1_begin;
  std::size_t buf1_buffer_offset = 0;

  Iterator2 buf2_iter = buf2_begin;
  std::size_t buf2_buffer_offset = 0;
...
buffer_compare(const ConstBufferSequence1& buf1,
  ...
  return detail::buffer_compare(
    ...
    buffer_sequence_begin(buf1),
    buffer_sequence_end(buf1),
    buffer_sequence_begin(buf2),
    buffer_sequence_end(buf2));
}
